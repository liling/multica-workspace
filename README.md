...
本仓库构建一个基于 Debian trixie 的镜像，预装 **opencode**、**hermes**、**multica** 三个 CLI 以及 **gstack**（Claude Code 技能集）运行环境，并附带 **openssh-client**（便于 git+ssh / 远程登录），容器启动后前台拉起 `multica daemon`，作为一台“设备”接入 Multica 平台、自动领取并执行分配给你的任务。容器默认工作目录为 `/workspace`，SSH 密钥目录 `/root/.ssh` 建议通过卷持久化（见下方「数据卷」）。
