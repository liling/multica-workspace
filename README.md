...
```dotenv
# 必填：Multica 登录令牌（daemon 实际消费的认证来源）
...
- **有令牌但未登录**：自动用 `MULTICA_TOKEN` 固化登录态（幂等；令牌无效也只是警告，daemon 本身会直接读环境变量认证）。
...
| 环境变量 | 必填 | 说明 |
|---|---|---|
| `MULTICA_TOKEN` | ✅ | 认证令牌（`mcn_...` Cloud Node PAT 或 `mul_...` 用户 PAT）。**daemon 实读的就是这个变量** |
...
- **设备没上线 / 日志报 `not authenticated`**：绝大多数情况是 `MULTICA_TOKEN` 没真正进到容器里。确认 `.env` 与 `docker-compose.yml` 同目录，并验证令牌已注入：
  ```bash
